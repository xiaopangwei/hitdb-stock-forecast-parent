package cn.hitdb.f2k.parser;

import cn.hitdb.core.model.message.BaseMessage;
import cn.hitdb.core.model.message.TickMessage;
...
import static cn.hitdb.f2k.model.Constant.*;
        ...
        for (String item : fieldNameList) {
            ...
            for (Field field : fields) {
                if (item.equals(field.getName())) {
                    ...
                    if (INT_CLASS.equals(classType)) {
                        byte[] intArray = ConvertUtils.int2Bytes((Integer) fieldValue);
                        System.arraycopy(intArray, 0, result, offset, INT_SIZE_IN_BYTE);
